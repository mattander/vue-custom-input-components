<template>
  <div :class="[wrapperClasses, errors.length > 0  && (!isClean || submitted) ? 'form-group--error' : '', 'form-group']">
    <label
      :for="inputId"
    ...
    <Typeahead
      :id="inputId"
      :value="value"
      @input="isClean = false; $emit('input', $event)"
      a
      @hit="hit = $event.payload"
      :disabled="disabled"
      :data="options"
    />
    <div
  ...
  computed: {
    errors() {
      ...
      if (this.value.length > 0 && this.value.trim().length == 0) {
        reqErrors.push("You can't use spaces as the only input for this field");
      }
